  ...
  const createMutation = useMutation({
    mutationFn: async (data: PrescriptionSlipFormData) => {
      try {
        ...
        console.log("[CreatePrescriptionSlip] Submitting data:", submitData);
        const response = await specifyVoteTestService.create(submitData);
        
        if (!response.success) {
          const errorMsg = response.error || response.message || "Không thể tạo phiếu chỉ định";
          console.error("[CreatePrescriptionSlip] Create failed:", errorMsg);
          throw new Error(errorMsg);
        }
        
        console.log("[CreatePrescriptionSlip] Create success:", response.data);
        return response;
      } catch (error: any) {
        console.error("[CreatePrescriptionSlip] Error in mutationFn:", error);
        throw error;
      }
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      console.error("[CreatePrescriptionSlip] onError:", error);
      const errorMessage = error?.message || error?.error || "Không thể tạo phiếu chỉ định. Vui lòng thử lại.";
      Alert.alert("Lỗi tạo phiếu chỉ định", errorMessage);
    },
  });
