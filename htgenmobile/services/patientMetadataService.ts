...
export const patientMetadataService = {
  getAll: async (params?: { page?: number; size?: number }): Promise<ApiResponse<PatientMetadataResponse[]>> => {
    try {
  ...
  /**
   * Get result presigned URL by patient ID
   * Backend: GET /api/v1/patient-metadata/result/{patientId}
   */
  getResultUrlByPatientId: async (
    patientId: string,
  ): Promise<ApiResponse<string>> => {
    try {
      const url = `/api/v1/patient-metadata/result/${encodeURIComponent(
        patientId,
      )}`;
      const response = await apiClient.get<string>(url);
      return response;
    } catch (error: any) {
      console.error("[PatientMetadataService] Error getting result URL:", error);
      return {
        success: false,
        error: error?.message || "Không thể lấy URL kết quả xét nghiệm",
      };
    }
  },
};
